...
class ResidualBlock(Layer):
  ...
  def __init__(self,
    ...
    Args:
      ...
      num_filters: The number of convolutional filters to use in this block
...
class TCN(Layer):
  """Creates a TCN layer.
    ...
    Args:
      num_filters: The number of filters to use in the convolutional layers. Can be a list.
      kernel_size: The size of the kernel to use in each convolutional layer.
      dilations: The list of the dilations. Example is: [1, 2, 4, 8, 16, 32, 64].
      num_stacks : The number of stacks of residual blocks to use.
  ...
  def __init__(self,
    ...
    if isinstance(self.num_filters, list):
      assert len(self.num_filters) == len(self.dilations)
      if len(set(self.num_filters)) > 1 and self.use_skip_connections:
  ...
  def receptive_field(self):
    return 1 + 2 * (self.kernel_size - 1) * self.num_stacks * sum(self.dilations)
  ...
  def build(self, input_shape):
    ...
    self.residual_blocks = []
    total_num_blocks = self.num_stacks * len(self.dilations)
    if not self.use_skip_connections:
      total_num_blocks += 1  # cheap way to do a false case for below

    for s in range(self.num_stacks):
      for i, d in enumerate(self.dilations):
        res_block_filters = self.num_filters[i] if isinstance(self.num_filters, list) else self.num_filters
        self.residual_blocks.append(ResidualBlock(dilation_rate=d,
                              num_filters=res_block_filters,
                              kernel_size=self.kernel_size,
                              padding=self.padding,
  ...
  def compute_output_shape(self, input_shape):
    ...
    if not self.return_sequences:
      ...
      num_filters = self.build_output_shape[-1]
  ...
  def get_config(self):
    ...
    config['num_stacks'] = self.num_stacks
...
def compiled_tcn(num_feat,  # type: int
  ...
  x = TCN(num_filters, kernel_size, num_stacks, dilations, padding,
